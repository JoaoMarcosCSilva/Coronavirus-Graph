...
def plot_minimum(Countries, minimum_dict, minimum_value, title = '', rows = 1, cols = 1, pos = 1, log = False, smooth = {'days' : 0}):
    C = Countries.copy()
    for c in C:
        if smooth['days'] != 0:
...
def main_plot(Countries, minimum_value = 100, minimum_title = 'case', log = False, row_mask = [True, True], col_mask = [True, True, True, True], smooth = {'days' : 0}):
    ...
    if row_mask[0]:
        if col_mask[0]:
            plot_minimum({c:Countries[c].daily_cases for c in Countries}, minimum_dict, minimum_value, 'Daily cases from day of %s %d' % (minimum_title, minimum_value), nrows, ncols, count, log, smooth)
            count += 1
        if col_mask[1]:
            plot_minimum({c:Countries[c].daily_deaths for c in Countries}, minimum_dict, minimum_value, 'Daily deaths from day of %s %d' % (minimum_title, minimum_value), nrows, ncols, count, log, smooth)
            count += 1
        if col_mask[2]:
            plot_minimum({c:Countries[c].daily_recover for c in Countries}, minimum_dict, minimum_value, 'Daily recoveries from day of %s %d' % (minimum_title, minimum_value), nrows, ncols, count, log, smooth)
            count += 1
        if col_mask[3]:
            plot_minimum({c:Countries[c].daily_active for c in Countries}, minimum_dict, minimum_value, 'Daily active cases from day of %s %d' % (minimum_title, minimum_value), nrows, ncols, count, log, smooth)
            count += 1
...
def main_plot_countries(Countries, names, minimum_value = 100, log = False, row_mask = [], col_mask = [], minimum_title = 'case', smooth = {'days' : 0}):
     main_plot({i:Countries[i] for i in names}, minimum_value, minimum_title, log, row_mask, col_mask, smooth)
